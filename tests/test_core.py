...
class TestIterdict(unittest.TestCase):
    ...
    def test_keyorder(self):
        data = {"a": [1, 2], "b": ["x", "y"]}
        results = list(productDict(data, keyorder=["b", "a"]))
        expected = [
            {"b": "x", "a": 1},
            {"b": "x", "a": 2},
            {"b": "y", "a": 1},
    ...
    def test_keyorder_validation(self):
        ...
        with self.assertRaises(ValueError):
            list(productDict(data, keyorder=["a"])) # Missing 'b'
        with self.assertRaises(ValueError):
            list(productDict(data, keyorder=["a", "c"])) # Wrong key 'c'

    def test_validation_not_dict(self):
        with self.assertRaises(TypeError):
            list(productDict("not a dict"))

    def test_list_and_enumerate(self):
        data = {"a": [1, 2], "b": [3, 4]}
        results = list(productDict(data))
        self.assertEqual(len(results), 4)
        for i, val in enumerate(productDict(data)):
